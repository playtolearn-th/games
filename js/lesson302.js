...
function preloadAllGameAudio(callback) {
  ...
  // เพิ่ม 'click' ในรายการเสียงที่จะโหลด
  const audioFiles = { 'correct': 'coin.mp3', 'wrong': 'wrong.mp3', 'win': 'winning.mp3', 'click': 'mouse-click.mp3' };
...
function displaySentence() {
  ...
  if (currentSentence.image) {
    imageContainer.innerHTML = `<img src="${imageBaseUrl}${currentSentence.image}" alt="รูปประกอบ">`;
  ...
  shuffledWords.forEach(word => {
    ...
    wordBtn.onclick = () => moveWord(wordBtn, wordOptions, sentenceBuilder);
...
function moveWord(wordBtn, fromContainer, toContainer) {
  playSound('click'); // เพิ่มเสียงคลิกที่นี่
  if (fromContainer === wordOptions) {
    wordBtn.onclick = () => moveWord(wordBtn, sentenceBuilder, wordOptions);
  } else {
    wordBtn.onclick = () => moveWord(wordBtn, wordOptions, sentenceBuilder);
  }
...
async function displayBestRank() {
  // This function is for displaying rank on the top bar, but this game doesn't have a rank system.
  // You can adapt it if you decide to add one. For now, it remains empty.
}
