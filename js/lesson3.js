...
const TOTAL_TIME = 120; // ปรับเวลาสำหรับ 8 คำ
const LIVES = 3;
let lives = LIVES;
let baseScore = 0, bonusScore = 0;
let timeLeft = TOTAL_TIME, completedLessons = 0;
const WORDS_PER_ROUND = 8; // แสดงทั้งหมด 8 คำในรอบเดียว
const imageBaseUrl = "images/";
const soundBaseUrl = "sound/"; // ใช้โฟลเดอร์ "sound" (ไม่มี s)
// อัปเดต: ใช้คำศัพท์ทั้ง 8 คำ
const allWordIds = ["ไก่", "ลูกช้าง", "ชู", "จ้อง", "ร้อง", "เรียก", "เพื่อน", "เด็ก"];
const allWords = allWordIds.map(id => ({ id: id, img: `${imageBaseUrl}${id}.png` }));
let matchedPairsInRound = 0;
let pairsInCurrentRound = 0;
const MAX_SCORE = 12080; // ปรับคะแนนสูงสุด (120s * 100 + 8pairs * 10)
...
function preloadAllGameAudio(callback) {
  ...
  // Add new word sounds to the preload list
  allWordIds.forEach(id => audioFiles[id] = `${id}.mp3`);
...
function startRound() {
  ...
  selected = [];
  const roundWords = [];
  // This loop will now take all words since WORDS_PER_ROUND is 8
  for (let i = 0; i < WORDS_PER_ROUND; i++) {
    if (gameWords.length === 0) break;
    const randomIndex = Math.floor(Math.random() * gameWords.length);
    const selectedWord = gameWords.splice(randomIndex, 1)[0];
...
function triggerRoundCompleteSequence() {
  ...
  setTimeout(() => {
    ...
    playSound('levelUp');
    completedLessons++;
    // Since there is only one round, go to final win sequence
    triggerFinalWinSequence();
  }, 600 + (coins.length * 50));
}
...
async function triggerFinalWinSequence() {
  ...
  if (currentUser) {
    // Changed Firestore document field to 'lesson3'
...
function calculateLessonLayout() {
  ...
  for (let cols = 2; cols <= 4; cols++) { // Adjusted for 8 items
...
function calculateAndApplyLayout() {
  ...
  const numCards = gameGrid.children.length; // Should be 16
  ...
  if (bestLayout.cols === 0) {
    const cols = 4; // Default to 4 columns for 16 cards
    const rows = Math.ceil(numCards / cols);
    const sizeW = (availableWidth - (cols - 1) * 10) / cols;
    const sizeH = (availableHeight - (rows - 1) * 10) / rows;
